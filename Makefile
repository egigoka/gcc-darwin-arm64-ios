...
build_alias=aarch64-apple-darwin20
build_vendor=apple
build_os=darwin20
...
target_alias=aarch64-apple-darwin20
target_vendor=apple
target_os=darwin20
...
TOPLEVEL_CONFIGURE_ARGUMENTS=./configure --disable-bootstrap --enable-languages='fortran c c++' --with-build-config=bootstrap-debug --prefix=/usr/bin/ --host=aarch64-apple-darwin20 --target=aarch64-apple-darwin20 --build=aarch64-apple-darwin20 --with-sysroot=/var/mobile/gcc/gcc-12.2.0/sdks-master/iPhoneOS14.5.sdk --with-gxx-include-dir=/usr/include/c++ --with-gmp-include=/usr/include --with-ar=/usr/bin/ar --with-gmp-lib=/usr/lib --disable-multilib

tooldir = ${exec_prefix}/aarch64-apple-darwin20
build_tooldir = ${exec_prefix}/aarch64-apple-darwin20
...
BUILD_SUBDIR = build-aarch64-apple-darwin20
# This is set by the configure script to the arguments to use when configuring
# directories built for the build system.
BUILD_CONFIGARGS =  --cache-file=./config.cache '--disable-bootstrap' '--enable-languages=fortran c c++' '--with-build-config=bootstrap-debug' '--prefix=/usr/bin/' '--with-sysroot=/var/mobile/gcc/gcc-12.2.0/sdks-master/iPhoneOS14.5.sdk' '--with-gxx-include-dir=/usr/include/c++' '--with-gmp-include=/usr/include' '--with-ar=/usr/bin/ar' '--with-gmp-lib=/usr/lib' '--disable-multilib' '--enable-languages=c,c++,fortran,lto' --program-transform-name='s,y,y,' --disable-option-checking --with-build-subdir="$(BUILD_SUBDIR)"
...
POSTSTAGE1_LDFLAGS = -static-libstdc++ -static-libgcc -nodefaultrpaths
...
SUBDIRS =  intl libiberty zlib libbacktrace libcpp libcody libdecnumber fixincludes gcc libcc1 c++tools
TARGET_CONFIGDIRS =  libgcc libbacktrace libgomp libatomic libitm libstdc++-v3 libssp libquadmath libgfortran
# This is set by the configure script to the arguments to use when configuring
# directories built for the host system.
HOST_CONFIGARGS =  --cache-file=./config.cache  '--disable-bootstrap' '--enable-languages=fortran c c++' '--with-build-config=bootstrap-debug' '--prefix=/usr/bin/' '--with-sysroot=/var/mobile/gcc/gcc-12.2.0/sdks-master/iPhoneOS14.5.sdk' '--with-gxx-include-dir=/usr/include/c++' '--with-gmp-include=/usr/include' '--with-ar=/usr/bin/ar' '--with-gmp-lib=/usr/lib' '--disable-multilib' '--enable-languages=c,c++,fortran,lto' --program-transform-name='s,y,y,' --disable-option-checking
# Host programs are put under this directory, which is . except if building
# with srcdir=..
HOST_SUBDIR = host-aarch64-apple-darwin20
# This is the list of variables to export in the environment when
...
TARGET_SUBDIR = aarch64-apple-darwin20
# This is set by the configure script to the arguments to use when configuring
# directories built for the target.
TARGET_CONFIGARGS = --cache-file=./config.cache   '--disable-bootstrap' '--enable-languages=fortran c c++' '--with-build-config=bootstrap-debug' '--prefix=/usr/bin/' '--with-sysroot=/var/mobile/gcc/gcc-12.2.0/sdks-master/iPhoneOS14.5.sdk' '--with-gxx-include-dir=/usr/include/c++' '--with-gmp-include=/usr/include' '--with-ar=/usr/bin/ar' '--with-gmp-lib=/usr/lib' '--disable-multilib' '--enable-languages=c,c++,fortran,lto' --program-transform-name='s,y,y,' --disable-option-checking --with-target-subdir="$(TARGET_SUBDIR)"
# This is the list of variables to export in the environment when
...
# Where to find GMP
HOST_GMPLIBS = -L/usr/lib -lmpc -lmpfr -lgmp
HOST_GMPINC = -I/usr/include 
...
configure-gcc: 
	...
	$(SHELL) \
	  ...
	  --srcdir=$${topdir}/$$module_srcdir \
	  $(HOST_CONFIGARGS) --build=${build_alias} --host=${host_alias} \
	  --target=${target_alias} --enable-pie-tools=yes \
	  || exit 1
